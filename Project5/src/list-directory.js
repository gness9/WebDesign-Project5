...
function listDirectory(req, res) {
  ...
  fs.readdir(dirPath, (err, entries) => {
      if(err) { console.log(err); return req.writeHead(404).end();}
      if(entries.includes('index.html')) { //For Index Implementation
       //serves the index file instead of the directory listing
